...
class Game {
  ...
  setOption(text) {
    ...
    if (bt_main.innerHTML == "Retirar") bt_main.style.backgroundColor = "#43a047";
    if (bt_main.innerHTML == "Aguarde") bt_main.style.backgroundColor = "#7b1fa2";
    if (bt_main.innerHTML == "Apostar") bt_main.style.backgroundColor = "#d81b60";
  }
  ...
  getTextButton() {
    return bt_main.innerHTML;
  }

  setTextButton(str) {
    bt_main.innerHTML = str;
  }

  checkWin() {
    if (this.mult_value == this.number && this.canTake && this.getTextButton() == "Retirar") {
      this.setEarns(this.bet_value + parseFloat((this.bet_value * this.number).toFixed(2)));
      this.msgWin(`<img width='100px' src='./imgs/bau.png'><p>R$ ${this.earns}</p>`);
      this.canTake = false;
      this.setTextButton("Apostar");
    }
  }
  ...
  takemoney() {
    if (this.pause) {
      this.setEarns(0);
      this.msgWin("Ganhou nada");
      return false;
    }

    if (this.canTake) {
      this.setEarns(parseFloat((this.bet_value * this.number).toFixed(2)));
      this.msgWin(`<img width='100px' src='./imgs/bau.png'><p>R$ ${this.earns}</p>`);
      this.canTake = false;
    }
  }
  ...
  start() {
    ...
    if (!this.pause) {
      ...
      if (this.number == this.number_limit) {
        ...
        if (this.getTextButton() == "Retirar") this.setTextButton("Apostar");

        this.pause = true;

        setTimeout(() => {
          this.message("Reiniciando,Reiniciando,6,5,4,3,2,1");
        }, 500);
      }

      bt_main.onclick = () => {
        if (this.getTextButton() == "Apostar") {
          this.bet_value_2 = this.bet_value;
          this.mult_value_2 = this.mult_value;
          this.setTextButton("Aguarde");
        }

        if (this.getTextButton() == "Retirar") {
          this.takemoney();
          this.setTextButton("Apostar");
        }
      };
  ...
  colorButton() {
    if (this.getTextButton() == "Retirar") bt_main.style.backgroundColor = "#43a047";
    if (this.getTextButton() == "Aguarde") bt_main.style.backgroundColor = "#7b1fa2";
    if (this.getTextButton() == "Apostar") bt_main.style.backgroundColor = "#d81b60";
  }
  ...
  reset() {
    ...
    if (this.getTextButton() == "Aguarde") bt_main.innerHTML = "Retirar";
  }
}
